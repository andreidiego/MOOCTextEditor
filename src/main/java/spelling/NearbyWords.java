...
public class NearbyWords implements SpellingSuggest {
	...
	public List<String> suggestions(String word, int numSuggestions) {
		...
		for (int i = 0; i < THRESHOLD && retList.size() <= numSuggestions && !queue.isEmpty(); i++) {
			String currentWord = queue.remove();
			List<String> wordsOneMutationAway = distanceOne(currentWord, true);
			for (String nextWordOneMutationAway : wordsOneMutationAway) {
				if(!visited.contains(nextWordOneMutationAway)) {
					visited.add(nextWordOneMutationAway);
					queue.add(nextWordOneMutationAway);
					if(dict.isWord(nextWordOneMutationAway))
						retList.add(nextWordOneMutationAway);
				}
			}
		}
